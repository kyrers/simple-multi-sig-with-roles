...
contract MetaMultiSigWallet is AccessControl {
    using ECDSA for bytes32;

    //Roles
    bytes32 public constant OG_ROLE = keccak256(abi.encode("OG_ROLE"));
    bytes32 public constant DEFAULT_ROLE = keccak256(abi.encode("DEFAULT_ROLE"));
    //------
    ...
    constructor(uint256 _chainId, address[] memory _owners, uint _signaturesRequired) public {
        ...
        //Give the contract the DEFAULT_ADMIN_ROLE so it can grant and revoke roles
    ...
    //Modifier to check that the role received is valid
    modifier validRole(string memory role) {
        bytes32 roleHash = keccak256(abi.encode(role));
        require(roleHash == OG_ROLE || roleHash == DEFAULT_ROLE, "Role not recognized!");
        _;
    }

    //Add Role parameter
    function addSigner(address newSigner, string memory role, uint256 newSignaturesRequired) public onlySelf validRole(role) {
        require(newSigner!=address(0), "addSigner: zero address");
        require(!isOwner[newSigner], "addSigner: owner not unique");
        require(newSignaturesRequired>0, "addSigner: new signatures required must be above 0");
